...
2. 只有使用 source，遇到`Error`才会触发 Rstudio 的自动呈现`trackback()`信息功能（详见\@ref(linear-process)）；
3. 只有使用 source 进入 debug 模式才会激活 Rstudio 的 debug toolbar（详见\@ref(nonlinear-process)）。

所以，在 debug 时，建议一律使用 source。
...
根据执行流程可以将代码分成两种情况，线性流程（linear process），非线性流程（nonlinear process，含有自己 defined function 或 control flow）。这两种情况下的第二步有些许不同。

#### Linear process {#linear-process}

1. 显性 bug 的第二步：在出错的地方停下来

在使用 source 或 Ctrl+A 然后 Ctrl+Enter 的方式运行线性流程的代码时，如果是显性 bug，Rstudio 会自动终止运行的代码，此时需要结合`Error`的信息定位 bug 的代码到底在哪一行。有两种基本方式：
...
- `traceback()`给出的只是一个相对粗略的 bug 定位，但如果 bug 是在代码中使用的某个 function 里，`traceback()`的指向会十分模糊，此时就需通过进入 debug 模式才能定位 bug（详见\@ref(nonlinear-process)）；
...
2. 显性 bug 的第三步：找到问题的根源（**关键**）

当将 bug 定位到具体某一行（称之为**起点行**）后，就需要确定引发 bug 的根源。知道为什么会有 bug，才能想出解决 bug 的方案。

精准定位的核心是“break it down into small parts”，大体可分为以下两个子步骤：

a. 分别检查**起点行**中所有可以查看的独立部分，确定到底是哪一个部分引发了 bug；
b. 如果问题的根源并不是这部分代码，则需要以倒序的方式，检查涉及该部分代码所操作的 object 的所有前序代码（同样也是分别检查所有可以查看的独立部分），看问题到底出在哪。

找到并解决 bug 最关键的便是上述第二步和第三步。

<video src="images/debug solve bug.mp4" width="100%" controls="controls"></video>

3. 隐性 bug 的第二步和第三步

相比于会自动实现第二步的显性 bug，隐性 bug 无法自动停止，所以需要根据出问题的 object（如和预期结果不同）倒推 bug 可能的位置和原因，使用的技术还是和显性 bug 一样，只不过更难。

<video src="images/debug implicit bug.mp4" width="100%" controls="controls"></video>

#### Nonlinear process {#nonlinear-process}

非线性流程指的是包含了自己 defined function （包括`apply()`函数族）或 control flow （如 loop structure）的代码。这种类型的代码和线性流程代码不同，因为 R 在执行时会将代码中的 defined function 和 control flow 部分视作是一个整体。一旦在这些部分部存在 bug 如`Error`时， R 虽然也会终止运行代码，但此时的`Error`信息并不足以让我们搞清楚到底问题在哪，特别是 defined function。所以就需要通过进入 debug 模式的方式实现第二步，在出错的地方地方停下来。

1. 隐性 bug 的第二步：在出错的地方停下来

`Error`类的显性 bug 同样也可以通过`traceback()`或搜索的方式来实现，就不再展示。重点讲解非线性流程中的隐性 bug。不同于显性 bug，隐性 bug 无法通过`traceback()`来定位，因为它并不会报`Error`。

<img style = "display: block; margin: auto;" src="images/debug implicit bug.png" width="100%"/>

容易看出，隐性 bug 是不会自己在出错的地方停下来，需要用户自己来决定在哪里停下来。而在哪里停下来实际上也就是寻找问题根源的过程，所以针对隐性 bug，第二步和第三步是合在一起的。

2. 隐性 bug 的第二步：找到问题的根源（**关键**）

通过在代码中键入`browser()`，保存并 source 即可进入 debug 模式。进入 debug 模式后，代码编辑窗口显示如下：
...
Debug 模式实际上就是用来暂停非线性流程的一种手段。进入 debug 模式以后，才能方便地使用前面介绍的两个子步骤来精准定位并解决 defined
function 或 control flow 内部的隐性 bug。例如：
...
#### Only use `browser()` in nonlinear process

如果把`browser()`放在线性代码中，next（F10）会直接把`browser()`后续的所有非 control flow 的代码全部运行完，但在 function 内部就会一行一行运行。

- 纯线性流程代码，next（F10）形同虚设。

<video src="images/debug browser in linear code.mp4" width="100%" controls="controls"></video>

- 含有 control flow 的线性流程代码，next（F10）只对 control flow 起作用。
...
通过以上的学习可以发现，debug 的一个关键环节就是**查看有问题代码的各个独立部分**。查看方式有两种：

1. 将这些独立部分的代码复制到 console 执行，适合查看执行**结果较为简短**的代码；
2. 还有一种就是通过`View()`在代码编辑窗口查看，适合查看执行**结果较庞杂**的代码。
