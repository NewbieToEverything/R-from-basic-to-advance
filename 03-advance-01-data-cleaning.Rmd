...
data_repeated_2f <- 
  ...
  cbind(id = 1:100) |>
  as.data.frame() |>
  setNames(c("id", "A1B1", "A2B1", "A1B2", "A2B2"))
...
数据清洗是为了将数据中的无效部分去除，并最终整理成一个清晰的形式，方便接下来直接使用统计分析方法处理清洗后的数据。因此，需要知道 R 中大多数统计分析函数对于数据的形式有什么样的要求，即什么样的数据才是干净、整洁的。这里介绍 Tidyverse 提出的 tidy data 的概念，包括以下三条规则：

1. Each variable must have its own column (most important).
...
当需要将`%>%`左侧的`x`传递给右侧的`fun`，且不是作为首个 argument 时，就需要用到占位符（placeholder）了，与`%>%`匹配的 placeholder 是`.`。那么如何理解占位符？

类似的，placeholder 就是先给以后要传递过来的`x`占个座，等`x`传过来了就它让作为这个位置的参数，然后执行`fun`。
...
```{r}
x <- 1
"x" %>% assign(4) # just like a function, %>% generates an additional env when evaluating, everything therefore happen in this environment, change will not be passed to the parent environments
x
"x" |> assign(4) # |> lead to direct assignment of x in the global environment because no additional env.
x
"x" |> (\(y) assign(y, 3))() #  anonymous function acts in a way similar to %>%, since function call awalys generate additional env
x
```
...
1. Wider format: One variable might be spread across multiple columns;
2. Longer format: One observation might be scattered across multiple rows.
...
- `names_sep`: `names_sep` takes the specification which can either be a numeric vector (specifying positions to break on), or a single string (specifying a regular expression to split on).
- `names_pattern`: `names_pattern` takes a regular expression containing matching groups (()).
...
head(data_repeated_2f)
data_repeated_2f |>
  pivot_longer(cols = names(data_repeated_2f)[2:5],
               names_to = c("A", "B"), 
               names_sep = 2, 
               values_to = "score")
```
...
# single string
names(data_repeated_2f)[2:5] <- c("A1_B1", "A2_B1", "A1_B2", "A2_B2")
head(data_repeated_2f)
data_repeated_2f |>
  pivot_longer(cols = names(data_repeated_2f)[2:5],
               names_to = c("A", "B"), 
...
head(df)
df$x <- recode_vars(df$x, max = 7, vars = c("x", "y"))
head(df)
df <- recode_vars(df, max = 7)
head(df)
```
