...
- `runif(n, min = 0, max = 1)`，生成`n`个在$[\min,\max]$这个闭区间（默认为$[0,1]$）内的均匀（uniform）分布随机数。
...
sample.int
```
核心函数为`.Internal(sample())`，对应 [random.c](https://github.com/SurajGupta/r-source/blob/a28e609e72ed7c47f6ddfbb86c85279a0750f0b7/src/main/random.c#L461) 中的`do_sample()`（第 461-553 行）。以最常见的`sample()`用法为例，如`sampe(1:5, size = 1, replace = FALSE, prob = NULL)`，对应执行的分支代码是`do_sample()`的最后一部分（533-547 行）：
```{c eval = FALSE}
int n = (int) dn;
...
#### A slightly deeper understanding of `set.seed()`
...
.Random.seed[1:5]
```
执行了`set.seed(1)`后将碰到的第一次含有随机行为的代码为`runif(1)`，这行代码共有一次随机行为，只要运行就会激活 RNG 一次，激活时会使用当前`.Random.seed`中储存的第 624 个 seed ，即 `r .Random.seed[626]` 作为起点。RNG 一旦被激活后，该种子就被使用了，`.Random.seed`第 2 个位置上记录的 下一次激活 RNG 将使用的 seed 位置序号就会自动 + 1，变为 1（因为一共就 624 个 seed，624 + 1 = 1）。

下一次含有随机行为的代码为`runif(2)`，共有两次随机行为，执行该行代码时会激活 RNG 两次：

1. 第一次将使用的起点就是第 1 个 seed，即 `r .Random.seed[3]`，RNG 激活完毕 seed 位置序号 + 1 变为 2；
2. 第二次将使用的起点就是第 2 个seed，即 `r .Random.seed[4]`，RNG 激活完毕 seed 位置序号 + 1 变为 3，即 `r .Random.seed[5]`，执行完毕又自动更新 seed 位置序号为 3，循环往复。

最后一次含有随机行为的代码为`runif(3)`。

由于 R 中`.Random.seed`第 2 个位置上存储的 seed 位置序号是由 R 自动记录 RNG 的激活次数并修改的，用户无法手动调整。所以，如果要控制 seed，用户唯一能做的就是通过`set.seed()`直接更换整套 624 seeds 并重置下一次激活 RNG 将使用的 seed 位置序号。

`set.seed()`能极大地提升代码运行结果的可重复性，因为只要`set.seed(integer)`中的`integer`不变，那后续所有含有随机行为的代码使用的 seed 顺序 就是固定的，即便是随机抽样也是可以完美重复出来的。
