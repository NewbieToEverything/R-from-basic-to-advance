...
R 里面所有与随机行为有关的代码（如`sample()`，`rnorm()`，`runif()`）本质上都是伪随机，都是通过某种随机数生成器（random number generator， RNG）来实现的。可以简单地认为每次 RNG 运行都需要给定一个起点，如果起点一致，那么两次随机行为的结果就是完全一致的。`set.seed()`就是用来控制起点——随机数种子（seed）的。

在 R 中，所有的 seed 都存储在`.Random.seed`这个 vector 里，不同的 RNG 产生的`.Random.seed`不一样，这里只讲 R 中默认的 RNG —— Mersenne-Twister。

Mersenne-Twister 的`.Random.seed`是一个长度为 626 的 integer vector，第 1 个位置上存储的是 RNG 的编号，第 3-626 个位置上存储了 624 个 seed。
```{r seed}
head(.Random.seed)
length(.Random.seed)
```
在不改变 RNG 种类的情况下，`set.seed()`只会改变 624 个 seed，并将`.Random.seed`第 2 个位置的数重置为 624。
```{r setseed}
set.seed(1)
head(.Random.seed)
set.seed(2)
head(.Random.seed)
```
当使用了`set.seed()`改变了 seed 后，运行的第一次含有随机行为的代码就会激活 RNG，并使用第 624 个 seed作为起点，这个被使用的 seed 位置序号 624 就是`.Random.seed`第 2 个位置上记录的信息。RN G一旦被激活一次，`.Random.seed`第 2 个位置上的记录的 seed 序号就会 + 1，变为 1，因为一共就 624 个 seed，624 + 1 = 1。下一次含有随机行为的代码在执行时，激活的 RNG 就会根据`.Random.seed`第 2 个位置上提供的 seed 序号自动使用第 1 个 seed 作为起点，其他以此类推。
```{r seed_position}
sample(1)
head(.Random.seed)
sample(1)
...
sample(1)
```
由于 R 中`.Random.seed`第 2 个位置上存储的下一个启用的 seed 序号是由 R 自动记录 RNG 的激活次数并修改的，用户无法手动调整。所以，如果要控制 seed，用户唯一能做的就是通过`set.seed()`直接更换整套 seed 并重置 seed 序号。
