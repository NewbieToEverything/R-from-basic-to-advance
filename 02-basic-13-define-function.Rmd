...
如果是定义 function，则基本结构保持不变，用`<-`给定义的 function 取个名字即可，
...
前面讲到，只要把`x <- value`的代码执行，就会把产生的 object 存到 environment 里去。同样的，只要把定义 function 的所有代码从上到下逐行执行，就会把产生的 function object 存到 environment 里去，然后就可以和正常的 function 一样调用了。
